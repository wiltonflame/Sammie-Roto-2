...
class SegmentationTab(QWidget):
    ...
    def _create_add_point_group(self, layout):
        ...
        self.object_spinbox.setToolTip(
            "Object ID for new points (each ID has its own color).\n\n"
            "SAM 2 tracks one mask and one memory stream per ID. If you put points on "
            "several separate things (e.g. two boxes, or a person and a prop) under the "
            "same ID, the model must force one mask to cover all of them—predictions get "
            "ambiguous and tracking flickers more. Use a different ID for each separate "
            "region or instance; combine them later in your compositor if needed."
        )
    ...
    def _create_tracking_group(self, layout):
        ...
        self.tracking_stabilize_checkbox = QCheckBox(
            "Stabilize tracked masks (less flicker between frames)"
        )
        self.tracking_stabilize_checkbox.setToolTip(
            "When tracking, blend each frame lightly with the previous one and smooth edges. "
            "Reduces temporal jitter; turn off if you need maximum sharpness on fast motion."
        )
        self.tracking_stabilize_checkbox.setChecked(
            get_settings_manager().get_session_setting("tracking_stabilize_masks", True)
        )
        self.tracking_stabilize_checkbox.stateChanged.connect(self._on_tracking_stabilize_changed)
        tracking_layout.addWidget(self.tracking_stabilize_checkbox)
    ...
    def load_values_from_settings(self):
        ...
        if hasattr(self, "tracking_stabilize_checkbox"):
            self.tracking_stabilize_checkbox.setChecked(
                settings_mgr.get_session_setting("tracking_stabilize_masks", True)
            )

    def _on_tracking_stabilize_changed(self):
        sm = get_settings_manager()
        sm.set_session_setting(
            "tracking_stabilize_masks", self.tracking_stabilize_checkbox.isChecked()
        )
        sm.save_session_settings()
        ...
        if count > 0:
            ...
            if device.type not in ("cuda", "mps"):
                show_message_dialog(
                    self, title="Error",
                    message=(
                        "CorridorKey needs a GPU: NVIDIA (CUDA) or Apple Silicon (MPS). "
            ...
            # Flush residual VRAM / unified memory from previous models (matting, etc.)
            import gc
            gc.collect()
            if torch.cuda.is_available():
        ...
        if framecount is not None and framecount > 0:
            ...
            if session_loaded:
                ...
                if points:
                    ...
                    # Warm SAM2 inference state from saved points (predictor starts empty
                    # after init_state; without this, the first new click sees no prior mask).
                    self.sam_manager.replay_points(points)

...
def _stabilize_propagated_mask_logits(
    logits_tensor,
    prev_prob,
    *,
    enable: bool,
    temporal_blend: float = 0.32,
    spatial_sigma: float = 0.85,
    morph_close_k: int = 3,
):
    """
    Turn SAM2 video-resolution mask logits into a uint8 mask. For propagated frames,
    optional temporal EMA (per object), Gaussian blur on probabilities, and a small
    morphological close reduce edge chatter and speckle between frames.
    Interactive segmentation paths keep enable=False.
    """
    prob = (
        torch.sigmoid(logits_tensor.float()).detach().cpu().numpy().astype(np.float32).squeeze()
    )
    if enable:
        if (
            prev_prob is not None
            and prev_prob.shape == prob.shape
            and temporal_blend > 0.0
        ):
            prob = (1.0 - temporal_blend) * prob + temporal_blend * prev_prob
        if spatial_sigma > 0.0:
            prob = cv2.GaussianBlur(prob, (0, 0), spatial_sigma)
    mask_u8 = ((prob > 0.5).astype(np.uint8)) * 255
    if enable and morph_close_k >= 3:
        k = morph_close_k if morph_close_k % 2 == 1 else morph_close_k + 1
...
class SamManager:
    ...
    def segment_image(self, frame_number, object_id, input_points, input_labels):
        ...
        if os.path.exists(frame_filename):
            # Do not reset_state here: resetting drops previous SAM outputs, so every
            # click re-runs with all points at once and no prev mask refinement (and
            # multimask is only used for a single point). Keeping state matches
            # interactive SAM2 and stabilizes multi-point segmentation. replay_points
    ...
    def track_objects(self, parent_window):
        ...
        stabilize = settings_mgr.get_session_setting("tracking_stabilize_masks", True)
        prev_prob_by_obj = {}
        ...
        for out_frame_idx, out_obj_ids, out_mask_logits in self.predictor.propagate_in_video(self.inference_state, start_frame_idx=in_point, max_frame_num_to_track=frames_to_track):
            for i, out_obj_id in enumerate(out_obj_ids):
                ...
                oid = int(out_obj_id)
                prev_p = prev_prob_by_obj.get(oid) if stabilize else None
                mask, prob_out = _stabilize_propagated_mask_logits(
                    out_mask_logits[i],
                    prev_p,
                    enable=stabilize,
                )
                if stabilize:
                    prev_prob_by_obj[oid] = prob_out
                os.makedirs(os.path.dirname(mask_filename), exist_ok=True)
                cv2.imwrite(mask_filename, mask)

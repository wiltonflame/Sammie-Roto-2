...
from corridorkey.paths import get_corridorkey_checkpoint_dir
...
def _torch_release_cache():
    """Free GPU memory on CUDA or Apple MPS (no-op on CPU-only)."""
    try:
...
class CorridorKeyManager:
    ...
    def load_model(self, parent_window=None):
        ...
        if device.type not in ("cuda", "mps"):
            print("CorridorKey requires a GPU (CUDA or Apple Silicon MPS)")
            return False
        ...
        try:
            ...
            default_ckpt = os.path.join(
                get_corridorkey_checkpoint_dir(), CORRIDORKEY_HF_FILE
            )
            checkpoint = getattr(self, "_checkpoint_override", default_ckpt)
            ...
            if quality in quality_to_imgsize:
                ...
                # Unified memory — avoid defaulting to tiny 1024 from missing CUDA props
                img_size = 1536
            else:
                try:
    ...
    def _ensure_model_downloaded(self, parent_window=None):
        ...
        ckpt_dir = get_corridorkey_checkpoint_dir()
        try:
            os.makedirs(ckpt_dir, mode=0o755, exist_ok=True)
        except OSError as e:
            print(
                f"CorridorKey: cannot create checkpoint directory {ckpt_dir!r}: {e}\n"
                f"Use a writable install location or check disk permissions."
            )
            return False

        ckpt_file = os.path.join(ckpt_dir, CORRIDORKEY_HF_FILE)
        ...
        try:
            for f in os.listdir(ckpt_dir):
                if f.endswith(".pth"):
                    self._checkpoint_override = os.path.join(ckpt_dir, f)
                    return True
        except OSError:
            pass
        ...
        dialog = _CorridorKeyDownloadDialog(
            CORRIDORKEY_HF_REPO, CORRIDORKEY_HF_FILE, ckpt_dir, parent_window
        )
        dialog.exec()
        ...
        try:
            ...
            for object_id in object_ids:
                ...
                for frame_num in range(start_frame, end_frame + 1):
                    ...
                    # Free VRAM / unified memory between frames
                    _torch_release_cache()
